...
def make_env(args, seed: int) -> ThesisPrimitiveSearchEnv:
    ...
    return ThesisPrimitiveSearchEnv(
        ThesisEnvConfig(
            ...
            reward_version=(
                f"thesis_{args.reward_mode}_{args.observation_frame}"
            ),
            ablation_name="thesis_v3_egocentric",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="egocentric",
        help=(
            "Use the original absolute maps or UAV-centred maps with the "
            "same observation dimensions."
        ),
    )
    parser.add_argument(
        "--reward-mode",
    ...
    parser.add_argument(
        ...
        type=float,
        default=10.0,
        help="Coefficient of coverage C(s) in Phi(s).",
    )
    parser.add_argument(
    ...
    # Accepted for runner symmetry; unused by the single-BDQN implementation.
...
def main() -> None:
    ...
    for episode in range(1, args.episodes + 1):
        ...
        while not done:
            ...
            # Both masks are required so Bellman targets never bootstrap
            # through an action that is invalid in the next state.
            agent.replay.add(
                obs,
                action,
                reward,
        ...
        row = {
            ...
            "validation_boundary": validation.get(
                "boundary_hit_ratio",
                np.nan,
            ),
            "validation_revisit": validation.get(
                "revisit_ratio",
                np.nan,
            ),
            "validation_sensor_revisit": validation.get(
                "sensor_revisit_ratio",
                np.nan,
            ),
            "validation_new_observed_per_step": validation.get(
        ...
        print(
            ...
            f"coverage="
            f"{float(validation.get('sensor_coverage_ratio_mean', np.nan)):.3f} "
